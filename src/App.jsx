...
// Componente separado para usuários normais (sem hooks condicionais)
function AppNormal() {
  ...
  const renderView = () => {
    ...
    return <div style={{ padding: 40 }}>Papel não reconhecido: {perfil?.papel}</div>
  }
...
// Componente para super admin
function AppSuperAdmin() {
  const { msg, show, toast } = useToast()
  return (
    <>
      <SuperAdmin onToast={toast} />
      <Toast msg={msg} show={show} />
    </>
  )
}
...
export default function App() {
  ...
  if (loading) return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center',
      height:'100vh', background:'var(--navy)', flexDirection:'column', gap:16 }}>
      <div style={{ width:48, height:48, borderRadius:14, background:'var(--emerald)',
      ...
      <div style={{ fontFamily:'var(--font-display)', fontSize:14, color:'rgba(255,255,255,.5)' }}>
        Carregando...
      </div>
    </div>
  )
